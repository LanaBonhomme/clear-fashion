  ...
  try {
    ...
    if(data.length == 0 || new Date(data.fetchDate).toISOString().split("T")[0] != new Date(Date.now()).toISOString().split("T")[0]) {
      ...
      const response = await fetch(
        `https://clear-fashion-theta-seven.vercel.app/`
      );
  ...
  const template = products
    .map(product => {
      ...
      <div class="product" id=${product._id}>
        ...
        <span style="color:#5b7e87">${product.brand}</span>
        <span>${product.name}</span>
        <span style="font-weight:bold">${product.price != null ? product.price + " €" : ""}</span></a>
        <span id="${product._id}-fav">`
      + ((JSON.parse(localStorage.getItem("favorites")) || []).includes(product._id) ? `<button onclick="deleteToFavorite(currentProducts[${i}]._id)">❤️</button>` : `<button onclick="addToFavorite(currentProducts[${i}]._id)">🤍</button>`) + `
      </span>
      </div></div>
    `;
    })
...
async function changeFavorites() {
  showOnlySelectFavorite.innerHTML = favoritesChecked ? "❤️" : "🤍";
